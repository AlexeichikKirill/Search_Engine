...
public class SearchServiceImpl implements SearchService {
    ...
    private List<Page> findPagesByQuery(String query, Site site) {
        SortedSet<Lemma> lemmaSortedSet = findLemmas(query, site);
        List<Page> pageList = new ArrayList<>();

        if (lemmaSortedSet.isEmpty()) {
            return new ArrayList<>();
        }
        this.lemmaSortedSet = lemmaSortedSet;
        List<Page> finalPageList = pageList;
        indexRepository.findAllByLemma(lemmaSortedSet.first()).forEach(index -> finalPageList.add(index.getPage()));
